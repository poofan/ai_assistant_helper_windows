...
class CoordinatesDialog(ctk.CTkToplevel):
    ...
    def create_button_widget(self, button_id: str, button_data: Dict):
        ...
        # Name entry
        name_label = ctk.CTkLabel(button_frame, text="Название:", width=80)
        ...
        coords_label = ctk.CTkLabel(button_frame, text="Координаты:", width=80)
        coords_label.pack(side="left", padx=5, pady=10)
        
        x_entry = ctk.CTkEntry(button_frame, placeholder_text="X", width=60)
        x_entry.insert(0, str(coords[0]) if len(coords) > 0 else "0")
        x_entry.pack(side="left", padx=2, pady=10)
        
        y_entry = ctk.CTkEntry(button_frame, placeholder_text="Y", width=60)
        y_entry.insert(0, str(coords[1]) if len(coords) > 1 else "0")
        y_entry.pack(side="left", padx=2, pady=10)
        
        w_entry = ctk.CTkEntry(button_frame, placeholder_text="W", width=60)
        w_entry.insert(0, str(coords[2]) if len(coords) > 2 else "100")
        w_entry.pack(side="left", padx=2, pady=10)
        
        h_entry = ctk.CTkEntry(button_frame, placeholder_text="H", width=60)
        h_entry.insert(0, str(coords[3]) if len(coords) > 3 else "50")
        h_entry.pack(side="left", padx=2, pady=10)
    ...
    def create_info_element_widget(self, element_id: str, element_data: Dict):
        """Create widget for an info element"""
        ...
        coords_text = f"X:{coords[0]}, Y:{coords[1]}, W:{coords[2]}, H:{coords[3]}" if len(coords) >= 4 else "Нет координат"
        coords_label = ctk.CTkLabel(
            element_frame,
            text=coords_text,
            width=200
        )
        coords_label.pack(side="left", padx=10, pady=10)
        
        # Description
        desc_label = ctk.CTkLabel(
            element_frame,
            text=element_data.get("description", ""),
            width=300
        )
...
class NewButtonDialog(ctk.CTkToplevel):
    ...
    def create_widgets(self):
        ...
        # Button ID
        id_label = ctk.CTkLabel(main_frame, text="ID кнопки:")
        ...
        # Button name
        name_label = ctk.CTkLabel(main_frame, text="Название:")
        ...
        # Coordinates
        coords_label = ctk.CTkLabel(main_frame, text="Координаты (X, Y, W, H):")
        ...
        self.w_entry = ctk.CTkEntry(coords_frame, placeholder_text="W", width=80)
        self.w_entry.pack(side="left", padx=5)
        self.w_entry.insert(0, "100")
        
        self.h_entry = ctk.CTkEntry(coords_frame, placeholder_text="H", width=80)
        self.h_entry.pack(side="left", padx=5)
        self.h_entry.insert(0, "50")
        
        # Description
        desc_label = ctk.CTkLabel(main_frame, text="Описание:")
